"""Модуль с хэндлерами незадействованными в машине состояний."""
...
async def command_start(message: types.Message) -> None:
    ...
    logger.info(
        f'Получена команда /start от пользователя {message.from_user.username}'
        f'id = {message.from_user.id}.')
    text = "Здравствуйте! Выберете категорию вопроса."
    logger.info(
        f'Попытка отправить сообщение: "{text}" для пользователя'
    ...
    try:
        ...
        logger.exception(
            f'Что то пошло не так при работе функции echo_start '
            f'ошибка: {error}')


async def handler_cancel(callback: types.CallbackQuery,
                         state: FSMContext) -> None:
    """Формирует ответ на кнопку Отменить."""
    current_state = await state.get_state()
    if current_state is None:
        await bot.send_message(chat_id=callback.from_user.id,
                               text="Сейчас нечего отменять, попробуйте "
...
async def random_text_message_handler(message: types.Message) -> None:
    ...
    Отвечает на текстовое сообщение пользователя.
    """
    logger.info(
        f'Получено сообщение:"{message.text}", от пользователя '
        f'{message.from_user.username} id = {message.from_user.id}.')
    try:
        ...
        logger.exception(
            f'Что то пошло не так при работе функции text_message_handler '
            f'ошибка: {error}')
...
async def faq_callback_key_handler(callback: types.CallbackQuery) -> None:
    ...
    key = callback.data
    logger.info(
        f'Нажата кнопка: "{key}" для пользователя'
    ...
    try:
        ...
        if isinstance(result, dict):  # Если результатом поиска является
            # словарь, формируется новое меню из кнопок, где для текста
            # кнопок используются ключи словаря result
            keyboard = menu_buttons(result)
            message = callback.message
            logger.info(
                f'Попытка отправить сообщение с новой клавиатурой для '
                f'пользователя {callback.from_user.username} '
            ...
            logger.info(
                f'Попытка отправить сообщение: "{result}" для пользователя '
                f'{callback.from_user.username} id = {callback.from_user.id}.')
        ...
        logger.exception(
            f'Что то пошло не так при работе функции callback_key '
            f'ошибка: {error}')
